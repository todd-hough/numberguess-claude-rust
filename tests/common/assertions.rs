...
pub fn assert_valid_game_response(game: &GameResponse) {
    assert!(game.game_id > 0, "Game ID should be positive, got {}", game.game_id);
    assert!(game.min <= game.max, "Min ({}) should be <= max ({})", game.min, game.max);
...
pub fn assert_game_in_range(game: &GameResponse, expected_min: u32, expected_max: u32) {
    assert_eq!(game.min, expected_min, "Game min should be {}, got {}", expected_min, game.min);
    assert_eq!(game.max, expected_max, "Game max should be {}, got {}", expected_max, game.max);
}
...
pub fn assert_valid_guess_response(response: &GuessResponse, expected_result: &str) {
    ...
    assert!(!response.message.is_empty(), "Guess response message should not be empty");
}

/// Assert that a "correct" guess response has all required fields
pub fn assert_correct_guess(response: &GuessResponse) {
    assert_eq!(response.result, "correct", "Result should be 'correct', got '{}'", response.result);
    assert!(response.attempts.is_some(), "Correct guess should include attempt count");
    assert!(
...
pub fn assert_incorrect_guess(response: &GuessResponse, expected_direction: &str) {
    ...
    assert!(!response.message.is_empty(), "Message should not be empty for incorrect guess");
}
